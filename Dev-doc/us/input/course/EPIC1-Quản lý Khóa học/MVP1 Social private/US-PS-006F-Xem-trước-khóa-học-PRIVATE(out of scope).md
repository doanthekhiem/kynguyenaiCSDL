...
- **Khi** Quản trị viên muốn xem khóa học như người học trên Online store
- **Thì** hệ thống mở tab/modal mới hiển thị trang khóa học với giao diện dành cho learner, hiển thị đầy đủ thông tin cơ bản của khóa học, thông tin về cấu trúc chương trình học (nếu đã có) trên màn hình, không cho phép enrollment thực sự trong preview mode, cho phép đóng preview để quay về trang admin
...
- **Thì** hệ thống hiển thị layout đầy đủ với placeholder: "Nội dung đang được xây dựng", cấu trúc chương trình rỗng với hướng dẫn, cho Admin hình dung được giao diện khi có nội dung, không trả về lỗi hoặc blank page
