...
|  |  | A |  |  |
|  |  | M | * AC-3:    + Danh sách nhóm khi thêm mới người dùng → chỉ hiển thị những nhóm có trạng thái "Đang hoạt động"   + Bổ sung thêm trường "Vai trò" để gán khi thêm mới nhóm | 22 |
|  |  | M | * Chỉnh sửa lại điều kiện bắt buộc cho trường "Vai trò" khi tạo mới | 28 |
|  |  | M | AC-3:   * Bổ sung thêm trường "Phương thức đăng nhập" | 30 |
...
Tôi muốn thêm mới người dùng vào trong tenant của mình

Để tôi có thể mở rộng đội ngũ và quản lý người dùng nội bộ theo vai trò cụ thể trong hệ thống giáo dục của tổ chức
...
Tại màn hình danh sách người dùng thuộc phân hệ Quản lý người dùng
...
| 5 | Phương thức đăng nhập | Có | * Dropdown chọn 1 giá trị * Danh sách giá trị:    + Email   + Số điện thoại |  |
| 6 | **Danh sách nhóm**   * Không bắt buộc * Danh sách tất cả {Nhóm} đã có trong tổ chức nhà trường/ trung tâm ở trạng thái "Đang hoạt động", ~~"Không hoạt động"~~ * Danh sách giá trị được sắp xếp theo ngày tạo từ mới nhất tới cũ nhất | | | |
| 7 | Thanh tìm kiếm | N/A | * Nhậ2p freetext * Placeholder "Tìm kiếm" * Quy định độ dài: 50 ký tự * Cho phép nhập ký tự là chữ cái, số, ký tự đặc biệt và khoảng trắng * Cho phép tìm kiếm theo: Tên nhóm, mã nhóm |  |
...
| 10 | Checkbox | Có | * Cho phép chọn nhiều nhóm cùng 1 lúc * Default: uncheck |  |
| 11 | Mã nhóm | N/A | * Hiển thị mã nhóm |  |
| 12 | Tên nhóm | N/A | * Hiển thị tên nhóm |  |
| 13 | Số lượng thành viên | N/A | * Hiển thị số lượng người dùng trong nhóm * Không có => hiển thị "--" |  |
| 14 | Trạng thái | N/A | * Hiển thị trạng thái của nhóm    + Đang hoạt động   + ~~Không hoạt động~~ |  |
| 15 | Ngày tạo | N/A | * Hiển thị thời điểm nhóm được tạo trên hệ thống * Format: dd/mm/yyyy |  |
...
Khi người dùng đang tick chọn nhóm rồi thực hiện tìm kiếm hoặc lọc theo2ngày
...
* Thông tin email:
  ...
  + Trong đó:
    ...
    - **{Tên người** 3**ùng}**: lấy tên người dùng theo email của người dùng được tạo mới
...
### AC-13: Gửi email thông báo user có quyền truy cập vào nhà trường/ trung tâm khi tạo mới thành công
...
* Thông tin email tự động:
  ...
  + Trong đó:
    ...
    - **{Tiêu đề email}**: Cấu hình tiêu đề gửi mail: Thông báo mở tài khoản – {**Tên đối tác**}
    - **{Email của nhà trường/ trung tâm}**: Cấu hình mail của trường/ trung tâm
    - **{Email của user}**: lấy email của chính người dùng đượ~~4~~ tạo mới.
    - **{Tên người dùng}**: lấy tên người dùng theo email của người dùng được tạo mới
    - {**Tên đối tác**}: Tên đối tác
    - **{username}**: Lấy email của user trong đối tác
    - **{link đăng nhập}**: là link truy cập vào đối tác theo subdomain
    - **{email CSKH}**: cấu hình email CSKH
    - **{số điện thoại}**: cấu hình số đi5n thoại: 0965856789
...
* Khi người dùng bấm "Hủy tạo mới để xác nhận"
  ...
  + Hệ thống sẽ:
    ...
    - Đóng popup
    - Hủy 7ỏ thao tác thêm mới
    - Không lưu bất kỳ thông tin gì trong form thêm mới người dùng
    - Quay trở lại màn hình danh sách loại tài nguyên
...
* Lỗi hệ thống nộ8 bộ (500 Internal Server Error)
...
| --- | --- | --- | --- |
| 1 | Mã định danh | * Tự động sinh * Unique |  |
| 2 | Loại định danh | * Mặc định Loại định danh khi tạo người dùng thành công = USER |  |
| 3 | Đối tác | * {Mã đối tác}-{Tên đối tác} người dùng thuộc về |  |
...
* **Tăng tốc độ mở rộng tổ chức:** Admin user có thể nhanh chóng thêm giáo viên, trợ giảng, nhân viên quản trị vào hệ thống.
...
* Ảnh hưởng khi US này được triển khai:

  + US Danh sách tài khoản định danh trên hệ thống EOP:

    - Khi tạo mới 1 user → Tự sinh identity tương ứng với user đó
    - Danh sách tài khoản định danh trên EOP được đồng bộ và cập nhật theo identity của user được tạo
